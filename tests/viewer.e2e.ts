...
test('browses a Pi session end to end on desktop and narrow screens', async ({ page }) => {
  ...
  await expect(llmOutput).toHaveCount(2)
  await expect(llmOutput.first().locator('.llm-output-header')).toHaveCount(0)
  await expect(llmOutput.first().locator('.event-row')).toHaveCount(4)
  await expect(llmOutput.first().locator('.event--tool-result')).toHaveCount(0)
  await expect(llmOutput.nth(1).locator('.event-row')).toHaveCount(1)
